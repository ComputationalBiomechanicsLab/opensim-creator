...
> Externally-sourced OpenSim plugins that are bundled included with `libopynsim`

This project builds a shared library containing OpenSim plugin components that
users have requested for inclusion into `libopynsim`.

The reason why plugins are included in-tree rather than (e.g.) providing a
native plugin API/UI is because `libopynsim`'s primary aim is to be an all-in-one
...
- If it becomes popular/useful, it's pulled into here, so that projects using
  `libopynsim` become responsible for ensuring it compiles and is available
  on all target platforms (typically, end-users/researchers only ensure
  their plugin works on Windows against one version of OpenSim)
