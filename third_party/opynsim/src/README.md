...
C++ code (`opynsim/_core/`). The C++ code uses `nanobind` to bind Python
to `libopynsim`.
...
- **The Python API is explicitly designed for Python, not automatically generated from C++**. It's
  harder to write bindings this way, but the resulting bindings feel more Python-native because
