...
TEST(CSV, read_row_reading_an_empty_stream_returns_a_single_empty_column)
{
    std::istringstream input;
    const std::optional<std::vector<std::string>> output = CSV::read_row(input);
...
TEST(CSV, read_row_reading_a_stream_containing_one_space_returns_a_single_column_containing_the_spaace)
{
    std::istringstream input{" "};
    const std::optional<std::vector<std::string>> output = CSV::read_row(input);
...
TEST(CSV, read_row_reading_a_stream_containing_just_two_commas_should_return_three_empty_columns)
{
    std::istringstream input{",,"};
    const  std::optional<std::vector<std::string>> output = CSV::read_row(input);
...
TEST(CSV, read_row_reading_a_stream_containing_standard_column_headers_returns_expected_output)
{
...
TEST(CSV, read_row_reading_a_stream_containing_multiple_lines_returns_each_row_as_expected)
{
    ...
    for (const auto& expected_output : expected_outputs) {
        const std::optional<std::vector<std::string>> columns = CSV::read_row(input);
        ASSERT_TRUE(columns.has_value());
        ASSERT_EQ(*columns, expected_output);
    }
}

TEST(CSV, read_row_reading_a_stream_containing_nested_quotes_works_as_expected_for_basic_example)
{
...
TEST(CSV, read_row_reading_a_stream_containing_nested_quotes_works_as_expected_for_example_exported_from_microsoft_excel)
{
...
TEST(CSV, read_row_reading_a_stream_after_eof_returns_std_nullopt)
{
    std::istringstream input{"col1,col2,col3"};

    const std::vector<std::string> expected_first_output = {"col1", "col2", "col3"};
    const std::optional<std::vector<std::string>> first_output = CSV::read_row(input);

    ASSERT_TRUE(first_output.has_value());
    ASSERT_EQ(*first_output, expected_first_output);

    const std::optional<std::vector<std::string>> second_output_after_eof = CSV::read_row(input);

    ASSERT_FALSE(second_output_after_eof.has_value());
}
TEST(CSV, read_row_edge_case_1)
{
...
TEST(CSV, read_row_edge_case_10)
{
...
TEST(CSV, write_row_writes_expected_content_to_stream_for_basic_example)
{
    const std::vector<std::string> input = {"a", "b", "c"};
    const std::string expected_output = "a,b,c\n";

    std::stringstream output;
    CSV::write_row(output, input);

    ASSERT_EQ(output.str(), expected_output);
}

TEST(CSV, write_row_writes_expected_content_to_stream_for_multiline_example)
{
...
TEST(CSV, write_row_edge_case_1)
{
    const std::vector<std::vector<std::string>> inputs = {
