#include "mouse_capturing_camera_v2.h"
...
void osc::MouseCapturingCameraV2::on_mount()
{
    grab_mouse(true);
}

void osc::MouseCapturingCameraV2::on_unmount()
{
    grab_mouse(false);
}

bool osc::MouseCapturingCameraV2::on_event(Event& e)
{
...
void osc::MouseCapturingCameraV2::on_draw()
{
    if (mouse_captured_) {
...
void osc::MouseCapturingCameraV2::grab_mouse(bool v)
{
    if (v) {
        if (not std::exchange(mouse_captured_, true)) {
            App::upd().set_main_window_relative_mouse_mode(true);
            App::upd().set_main_window_mouse_confinement(0.5f*App::get().main_window_dimensions());
        }
    }
    else {
        if (std::exchange(mouse_captured_, false)) {
            ...
            App::upd().set_main_window_relative_mouse_mode(false);
        }
    }
}
