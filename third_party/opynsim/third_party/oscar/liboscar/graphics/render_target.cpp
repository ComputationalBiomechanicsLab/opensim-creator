...
{
    const auto required_dimensions = pixel_dimensions();
    ...
    for (const auto& color_attachment : colors_) {
        OSC_ASSERT_ALWAYS(color_attachment.buffer.pixel_dimensions() == required_dimensions);
        OSC_ASSERT_ALWAYS(color_attachment.buffer.anti_aliasing_level() == required_aa_level);
    }
    if (depth_) {
        OSC_ASSERT_ALWAYS(depth_->buffer.pixel_dimensions() == required_dimensions);
