...
#define pg_c_1 pg_b_1
  ...
  for (size_t k = 0; k < shared_dim; k++) {
    ...
    // Load row of B**T
    svfloat32_t row_b_0 = svld1(pg_b_0, &B_T[k * svl]);
    svfloat32_t row_b_1 = svld1(pg_b_1, &B_T[(k + shared_dim) * svl]);
#else
    // Computes alpha*A**T*B
    // Load column of A**T
    ...
    // Load row of B
    svfloat32_t row_b_0 = svld1(pg_b_0, &B_T[k * ldb]);
    svfloat32_t row_b_1 = svld1(pg_b_1, &B_T[k * ldb + svl]);
#endif
...
#if !defined(TRANSA)
    // Computes alpha*B*A**T
    // Load column of B
    col_a_0 = svld1(pg_a_0, &B[k * svl]);
    col_a_0 = svmul_x(pg, alpha_vec, col_a_0);
    col_a_1 = svld1(pg_a_1, &B[(k + shared_dim) * svl]);
    col_a_1 = svmul_x(pg, alpha_vec, col_a_1);

    // Load row of A**T
    row_b_0 = svld1(pg_b_0, &A_T[k * svl]);
    row_b_1 = svld1(pg_b_1, &A_T[(k + shared_dim) * svl]);
#else
    // Computes alpha*B**T*A
    // Load column of B**T
    col_a_0 = svld1(pg_a_0, &B[k * ldb]);
    col_a_0 = svmul_x(pg, alpha_vec, col_a_0);

    col_a_1 = svld1(pg_a_1, &B[k * ldb + svl]);
    col_a_1 = svmul_x(pg, alpha_vec, col_a_1);

    // Load row of A
    row_b_0 = svld1(pg_b_0, &A_T[k * ldb]);
    row_b_1 = svld1(pg_b_1, &A_T[k * ldb + svl]);
#endif
    // Perform outer product
    svmopa_za32_m(/*tile*/0, pg, pg, col_a_0, row_b_0);
    svmopa_za32_m(/*tile*/1, pg, pg, col_a_0, row_b_1);
...
static void ssyr2k_direct_sme1_2VLx2VL(uint64_t n, uint64_t k, const float* alpha,\
                                   const float *ba, const float *bb, const float* beta, float *restrict bc) {
  ...
  for (; row_idx < num_rows; row_idx += row_batch) {
    ...
    for (uint64_t col_idx = 0; col_idx < num_cols && col_idx <= row_idx; col_idx += col_batch) {
      ...
      col_batch = MIN(col_batch, num_cols - col_idx);
#if !defined(TRANSA)
      kernel_2x2(&a_ptr[row_idx * k], &b_ptr[col_idx * k], &b_ptr[row_idx * k], &a_ptr[col_idx * k],
                 &c_ptr[row_idx * ldc + col_idx], k,
				 ldc, row_batch, col_batch, *alpha, *beta, row_idx, col_idx);
#else
      kernel_2x2(&a_ptr[row_idx], &b_ptr[col_idx], &b_ptr[row_idx], &a_ptr[col_idx],
                &c_ptr[row_idx * ldc + col_idx], k,
                ldc, row_batch, col_batch, *alpha, *beta, row_idx, col_idx);
#endif
            ...
            float beta, float * __restrict R, BLASLONG strideR)
{
        ...
        ssyr2k_direct_sme1_2VLx2VL(N, K, &alpha, A_mod, B_mod, &beta, R);
        free(A_mod);
        free(B_mod);
#else
        ssyr2k_direct_sme1_2VLx2VL(N, K, &alpha, A, B, &beta, R);
#endif
}
#else
...
void CNAME (BLASLONG N, BLASLONG K, float alpha, float * __restrict A, \
            BLASLONG strideA, float * __restrict B, BLASLONG strideB, \
            float beta, float * __restrict C, BLASLONG strideC){}
