...
LDFLAGS="-fexceptions -sNO_DISABLE_EXCEPTION_CATCHING=1 -sLEGACY_GL_EMULATION=1 -sUSE_WEBGL2=1 -sUSE_SDL=2" CXXFLAGS="-fexceptions --use-port=sdl2" CC=emcc CXX=em++ cmake -S . -B osc-build -DCMAKE_EXECUTABLE_SUFFIX=".html" -DOSC_BUILD_OPENSIMCREATOR=OFF -DOSC_DISCOVER_TESTS=OFF -DOSC_EMSCRIPTEN=ON -DCMAKE_PREFIX_PATH=${PWD}/osc-deps-install
