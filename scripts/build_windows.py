...
def build_osc_dependencies(conf: BuildConfiguration):
    with Section("build osc dependencies"):
        _run(
            f'cmake -S third_party/ -B "{conf.get_dependencies_build_dir()}" {conf.generator_flags} -DCMAKE_BUILD_TYPE={conf.get_osc_deps_build_type()} -DCMAKE_INSTALL_PREFIX="{conf.get_dependencies_install_dir()}" -DOSCDEPS_GET_OPENBLAS=ON',
