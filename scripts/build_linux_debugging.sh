...
CCFLAGS="-fsanitize=address -fno-sanitize-recover=all" CXXFLAGS="-fsanitize=address -fno-sanitize-recover=all" LDFLAGS="-rdynamic" cmake -S . -B osc-build -DCMAKE_BUILD_TYPE=RelWithDebInfo -DOSC_FORCE_ASSERTS_ENABLED=ON -DCMAKE_PREFIX_PATH=${PWD}/osc-deps-install -DCMAKE_INSTALL_PREFIX=${PWD}/osc-install -DCMAKE_CXX_CLANG_TIDY=${CLANG_TIDY}
