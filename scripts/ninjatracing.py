...
OPENSIM CREATOR GUIDE:

- Compile project with ninja (e.g. with Visual Studio's in-built CMake+Ninja support)
- Find `.ninja_log` in the build dir
- Provide as an arg to this
"""
...
def main(argv):
    ...
    # OPENSIM CREATOR: modifications here to make it easier to read in a terminal
    all_paths = [e['name'] for e in entries]
    prefix = os.path.commonpath(all_paths)
    for el in sorted(entries, key=lambda entry: entry['dur'], reverse=True):
        dur = el['dur']/1000000  # convert to seconds
        printed_path = os.path.relpath(el['name'], prefix)
        print(f"{dur: 8}\t{printed_path}")
