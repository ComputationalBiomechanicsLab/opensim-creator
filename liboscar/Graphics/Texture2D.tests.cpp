...
TEST(Texture2D, default_constructor_creates_sRGBA_texture_with_expected_params)
{
    ...
    ASSERT_EQ(texture_2d.pixel_dimensions(), Vec2i(1, 1));
...
{
    const Vec2i pixel_dimensions = {1, 1};
...
TEST(Texture2D, set_pixel_data_with_32bit_floating_point_components_followed_by_get_pixels_returns_same_span)
{
    ...
    const Vec2i pixel_dimensions = {1, 1};
    std::vector<Vec4> const rgba_float32_pixels(static_cast<size_t>(pixel_dimensions.x * pixel_dimensions.y), color);
...
TEST(Texture2D, dimensions_equal_pixel_dimensions_on_construction)
{
    ...
    ASSERT_EQ(texture_2d.pixel_dimensions(), Vec2i(7, 7));
    ASSERT_EQ(Vec2(texture_2d.pixel_dimensions()), texture_2d.dimensions());
}
...
TEST(Texture2D, dimensions_are_scaled_by_device_pixel_ratio)
{
    ...
    ASSERT_EQ(texture_2d.dimensions(), Vec2(7.0f, 7.0f));
    texture_2d.set_device_pixel_ratio(2.0f);
    ASSERT_EQ(texture_2d.dimensions(), Vec2(7.0f,7.0f)/2.0f);
    texture_2d.set_device_pixel_ratio(0.5f);
    ASSERT_EQ(texture_2d.dimensions(), Vec2(7.0f,7.0f)/0.5f);
}
