...
OpenSim includes a user interface (UI) that can edit the parameters of existing OpenSim models, but it is specialized around tools that accept existing model files as input. For example, it includes tools for scaling models, computing inverse kinematics (IK) from marker motion recordings, and computed muscle control (CMC), but it does not include tools for adding bodies or joints to an existing model. Therefore, to create new models, users typically use a combination of the following three options:
