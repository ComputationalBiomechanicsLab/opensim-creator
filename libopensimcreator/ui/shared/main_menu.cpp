...
#include <libopensimcreator/documents/simulation/sto_file_simulation.h>
...
namespace
{
    void LoadMotionAgainstModel(const std::shared_ptr<opyn::ModelStatePair>& model, Widget* parent)
    {
        if (not model) {
...
void osc::MainMenuFileTab::onDraw(std::shared_ptr<opyn::ModelStatePair> maybeModel)  // NOLINT(performance-unnecessary-value-param)
{
...
void osc::MainMenuAboutTab::onDraw()
{
    ...
    ui::draw_vertical_spacer(0.5f/15.0f);
    {
        ...
        bool enabled = App::get().vsync_enabled();
    ...
    int id = 0;
    {
        ...
        ui::next_column();
        {
            bool appIsInDebugMode = App::get().debug_mode();
