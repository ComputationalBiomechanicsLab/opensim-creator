...
TEST(Mesh, SetIndicesRecalculatesBounds)
{
    ...
    ASSERT_EQ(m.getBounds(), bounding_aabb_of(triangle));
}
...
TEST(Mesh, GetBooundsReturnsNonemptyForIndexedVerts)
{
    ...
    ASSERT_EQ(m.getBounds(), bounding_aabb_of(pyramid));
}
...
TEST(Mesh, SetVertexBufferDataRecalculatesBounds)
{
    ...
    ASSERT_EQ(m.getBounds(), bounding_aabb_of(firstVerts));

    m.setVertexBufferData(secondVerts);

    ASSERT_EQ(m.getBounds(), bounding_aabb_of(secondVerts));
}
