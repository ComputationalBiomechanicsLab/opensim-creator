...
static std::optional<AbstractPropertyEditor::Response> draw_editor(
    ...
    for (int idx = 0; idx < prop.size(); ++idx) {
        ...
        if (!prop.isOneValueProperty()) {
            if (ImGui::Button("X")) {
                if (!rv) {
                    auto deleter = [idx](OpenSim::AbstractProperty& p) {
                        auto* ps = dynamic_cast<OpenSim::SimpleProperty<std::string>*>(&p);
                        if (!ps) {
                            return;
                        }
                        auto copy = std::make_unique<OpenSim::SimpleProperty<std::string>>(ps->getName(), ps->isOneValueProperty());
                    ...
                    rv = AbstractPropertyEditor::Response{deleter};
                }
            }
            ImGui::SameLine();
        }
