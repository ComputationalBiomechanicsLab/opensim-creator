    ...
    Vec3 locationInGround)
{
    ...
    ui::SameLine();
    ui::InputVec3("##translation", position, "%.6f", ImGuiInputTextFlags_ReadOnly);
}
    ...
    Vec3 directionInGround)
{
    ...
    ui::SameLine();
    ui::InputVec3("##direction", direction, "%.6f", ImGuiInputTextFlags_ReadOnly);
}
    ...
    OpenSim::Frame const& otherFrame)
{
    ...
    ui::SameLine();
    ui::InputVec3("##orientation", rotationEulers, "%.6f", ImGuiInputTextFlags_ReadOnly);
}
    ...
    OpenSim::Frame const& frame)
{
    if (ui::BeginMenu("Axis Directions")) {
        auto const onFrameMenuOpened = [&state, &frame](OpenSim::Frame const& other)
        {
            ...
            ui::Text("x axis");
            ui::SameLine();
            ui::InputVec3("##xdir", x, "%.6f", ImGuiInputTextFlags_ReadOnly);

            ui::Text("y axis");
            ui::SameLine();
            ui::InputVec3("##ydir", y, "%.6f", ImGuiInputTextFlags_ReadOnly);
    ...
    OpenSim::Ellipsoid const& ellipsoid)
{
    if (ui::BeginMenu("Radii")) {
        ...
        ui::Text("radii");
        ui::SameLine();
        ui::InputVec3("##radii", v, "%.6f", ImGuiInputTextFlags_ReadOnly);
        ui::EndMenu();
    }
}
    ...
    std::function<void(std::optional<std::filesystem::path>)> const& onUserClickedOpenOrSelectedFile)
{
    ...
    if (ui::BeginPopupContextItem("##RecentFilesMenu", ImGuiPopupFlags_MouseButtonLeft))
    {
