...
namespace {
    ...
    inline constexpr size_t filename_offset(const T(&str)[S], size_t i = S - 1) noexcept {
        if (i == 0) {
            return 0;
        }
...
namespace osc {
    ...
    [[noreturn]] void on_assertion_failure(char const* failing_code, char const* func, char const* file, unsigned int line) noexcept;
}

// macro that produces just the filename (rather than the full filepath)
#define OSC_FILENAME &__FILE__[::const_expr_val<size_t, filename_offset(__FILE__)>::value]

// always execute this assertion - even if in release mode /w debug flags disabled
#define OSC_ASSERT_ALWAYS(expr)                                                                                       \
    (static_cast<bool>(expr) ? (void)0 : osc::on_assertion_failure(#expr, __func__, OSC_FILENAME, __LINE__))
