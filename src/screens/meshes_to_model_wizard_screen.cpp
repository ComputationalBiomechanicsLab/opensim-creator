...
[[nodiscard]] static Mesh_load_response create_response_v2(Mesh_load_request const& msg) noexcept {
    try {
...
// MESH LOADER MAIN FN
static int mesh_loader_thread_main_v2(
        osc::stop_token,
        spsc::Receiver<Mesh_load_request> rx,
        spsc::Sender<Mesh_load_response> tx) {
    ...
    while (!tx.is_receiver_hung_up()) {
        ...
        tx.send(create_response_v2(*msg));
    }
...
struct Mesh_loaderv2 final {
    ...
    static Mesh_loaderv2 create() {
        auto [req_tx, req_rx] = spsc::channel<Mesh_load_request>();
        auto [resp_tx, resp_rx] = spsc::channel<Mesh_load_response>();
        osc::jthread worker{mesh_loader_thread_main_v2, std::move(req_rx), std::move(resp_tx)};
        return Mesh_loaderv2{std::move(worker), std::move(req_tx), std::move(resp_rx)};
    }
...
struct osc::Meshes_to_model_wizard_screen_v2::Impl final {
    // loader that loads mesh data in a background thread
    Mesh_loaderv2 mesh_loader = Mesh_loaderv2::create();
...
using Impl = osc::Meshes_to_model_wizard_screen_v2::Impl;
...
[[nodiscard]] static glm::vec3 center(Loaded_mesh const& lm) noexcept {
...
[[nodiscard]] static bool is_bof_connected_to_ground(Impl& impl, int bofidx) {
    int jumps = 0;
    while (bofidx >= 0) {
...
static void test_for_advancement_issues(Impl& impl) {
...
static void recursively_add_bof(
    ...
    if (!bof.is_frame) {
        ...
        for (Loaded_mesh const& m : impl.meshes) {
            ...
            SimTK::Transform mmtx = to_transform(m.model_mtx);
...
static void try_creating_output_model(Impl& impl) {
...
static void set_is_selected_of_all_to(Impl& impl, bool v) {
...
static void set_is_hovered_of_all_to(Impl& impl, bool v) {
    for (Loaded_mesh& m : impl.meshes) {
        m.is_hovered = v;
    }
...
static void set_hovered_els_as_selected(Impl& impl) {
    for (Loaded_mesh& m : impl.meshes) {
...
static void submit_meshfile_load_request(Impl& impl, std::filesystem::path path) {
...
static void prompt_user_to_select_multiple_mesh_files(Impl& impl) {
    nfdpathset_t s{};
...
static void update_camera_from_user_input(Impl& impl) {
...
static void action_delete_selected(Impl& impl) {
...
// add frame to model
static void action_add_frame(Impl& impl, glm::vec3 pos) {
...
// add body to model
static void action_add_body(Impl& impl, glm::vec3 pos) {
...
static void update_impl_from_user_input(Impl& impl) {
...
static glm::vec2 world2screen(Impl& impl, glm::vec3 const& v) {
...
static void draw_bof_line_to_parent(Impl& impl, Body_or_frame const& bof) {
...
static void draw_2d_overlay(Impl& impl) {
...
static void draw_mesh_hover_tooltip(Impl&, Loaded_mesh& m) {
    ImGui::BeginTooltip();
...
static void draw_ground_hover_tooltip(Impl&) {
    ImGui::BeginTooltip();
    ImGui::Text("Ground");
...
static void draw_bof_hover_tooltip(Impl&, Body_or_frame& bof) {
    ImGui::BeginTooltip();
...
static void draw_mesh_context_menu_content(Impl& impl, Loaded_mesh& lum) {
...
static void draw_ground_context_menu_content(Impl& impl) {
    if (ImGui::MenuItem("add body")) {
        action_add_body(impl, {0.0f, 0.0f, 0.0f});
    }
...
static void draw_3d_scene(Impl& impl) {
...
static void handle_hovertest_result(Impl& impl) {
...
static void handle_hovertest_result_assignment_mode(Impl& impl) {
...
static void draw_scene_context_menu(Impl& impl) {
...
// bodies)
static void draw_standard_3d_viewer(Impl& impl) {
...
static void draw_assignment_3d_viewer(Impl& impl) {
...
// draw 3D viewer
static void draw_3dviewer(Impl& impl) {
...
static void draw_sidebar(Impl& impl) {
...
static void draw_screen(Impl& impl) {
    ImGuizmo::BeginFrame();
...
static void pop_mesh_loader_output_queue(Impl& impl) {
...
osc::Meshes_to_model_wizard_screen_v2::Meshes_to_model_wizard_screen_v2() :
    impl{new Impl{}} {
}

osc::Meshes_to_model_wizard_screen_v2::Meshes_to_model_wizard_screen_v2(std::vector<std::filesystem::path> paths) :
    impl{new Impl{}} {
...
osc::Meshes_to_model_wizard_screen_v2::~Meshes_to_model_wizard_screen_v2() noexcept = default;

void osc::Meshes_to_model_wizard_screen_v2::draw() {
    ::draw_screen(*impl);
}

void osc::Meshes_to_model_wizard_screen_v2::tick(float) {
    ::tick(*impl);
}
