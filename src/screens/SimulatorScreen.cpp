...
struct osc::SimulatorScreen::Impl final {
    ...
    // ui component state
    osc::ui::log_viewer::State logViewerState;
    osc::ui::main_menu::file_tab::State mmFiletabState;
...
#define OSC_DRAW_SIMSTAT_PLOT(statname) \
    {                                                                                                                  \
        ...
        ImGui::SameLine();  \
        ui::help_marker::draw(SimStats::g_##statname##Desc);  \
        ImGui::NextColumn();  \
    ...
    void simscreenDraw(osc::SimulatorScreen::Impl& impl) {
        ...
        if (ImGui::BeginMainMenuBar()) {
            ui::main_menu::file_tab::draw(impl.mmFiletabState, impl.mes);
        ...
        if (st.showing.log) {
            if (ImGui::Begin("Log", &st.showing.log, ImGuiWindowFlags_MenuBar)) {
                ui::log_viewer::draw(impl.logViewerState);
            }
            ImGui::End();
        }
    }
