...
#include "src/opensim_bindings/FileChangePoller.hpp"
#include "src/opensim_bindings/OpenSimHelpers.hpp"
...
struct ModelEditorScreen::Impl final {
    ...
    struct {
        ui::main_menu::file_tab::State mainMenuTab;
        AddBodyPopup addBodyPopup;
        ui::properties_editor::State propertiesEditor;
        ui::reassign_socket::State reassignSocket;
        AttachGeometryPopup attachGeometryPopup;
        ui::select_2_pfs::State select2pfs;
        ui::model_actions::State modelActions;
        ui::log_viewer::State logViewer;
    } ui;
...
namespace {
    ...
    void drawHCFContextualActions(
        ...
        // render standard, easy to render, props of the contact params
        {
            ...
            auto maybe_updater = ui::properties_editor::draw(st, params, easyToHandleProps);
    ...
    void drawPathActuatorContextualParams(
        ...
        // handle popup
        {
            OpenSim::PhysicalFrame const* pf = ui::select_1_pf::draw(modalName, uim.model());
            if (pf) {
    ...
    void drawSocketEditor(ModelEditorScreen::Impl& impl, UndoableUiModel& uim) {
        ...
        for (std::string const& sn : socknames) {
            ...
            if (OpenSim::Object const* connectee =
                    ui::reassign_socket::draw(impl.ui.reassignSocket, popupname.c_str(), uim.model(), socket);
                connectee) {
                ...
                try {
                    ...
                    impl.ui.reassignSocket.search[0] = '\0';
                    impl.ui.reassignSocket.error.clear();
                    ImGui::CloseCurrentPopup();
                } catch (std::exception const& ex) {
                    impl.ui.reassignSocket.error = ex.what();
    ...
    void drawSelectionEditor(ModelEditorScreen::Impl& impl, UndoableUiModel& uim) {
        ...
        ImGui::Separator();
        {
            auto maybeUpdater = ui::properties_editor::draw(impl.ui.propertiesEditor, *uim.getSelection());
            if (maybeUpdater) {
                uim.beforeModifyingModel();
    ...
    void drawMainMenu(ModelEditorScreen::Impl& impl) {
        if (ImGui::BeginMainMenuBar()) {
            ui::main_menu::file_tab::draw(impl.ui.mainMenuTab, impl.st);
    ...
    void modelEditorDrawUnguarded(ModelEditorScreen::Impl& impl) {
        ...
        if (impl.st->showing.actions) {
            if (ImGui::Begin("Actions", nullptr, ImGuiWindowFlags_MenuBar)) {
                ...
                ui::model_actions::draw(
                    impl.ui.modelActions, impl.st->editedModel.model(), onSetSelection, onBeforeModifyModel, onAfterModifyModel);
            }
            ImGui::End();
        }
        ...
        if (impl.st->showing.selectionDetails) {
            if (ImGui::Begin("Selection", &impl.st->showing.selectionDetails)) {
                auto resp = ui::component_details::draw(impl.st->editedModel.state(), impl.st->editedModel.getSelection());
        ...
        if (impl.st->showing.log) {
            if (ImGui::Begin("Log", &impl.st->showing.log, ImGuiWindowFlags_MenuBar)) {
                ui::log_viewer::draw(impl.ui.logViewer);
            }
            ImGui::End();
        }
        ...
        // draw sim params editor popup (if applicable)
        {
            ...
            osc::ui::fd_params_editor_popup::draw("simulation parameters", impl.st->simParams);
        }
