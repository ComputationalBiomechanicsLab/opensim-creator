...
namespace
{
    ...
    std::map<std::string, Mesh> GenerateMeshLookup()
    {
        ...
        return {
            ...
            {"cube2 (wire)", WireframeGeometry::generate_mesh(BoxGeometry::generate_mesh(2.0f, 2.0f, 2.0f, 1, 1, 1))},
            {"yline", cache.getYLineMesh()},
            {"quad", cache.getTexturedQuadMesh()},
            {"torus", cache.getTorusMesh(0.9f, 0.1f)},
            {"torusknot", TorusKnotGeometry::generate_mesh()},
            {"box", BoxGeometry::generate_mesh(2.0f, 2.0f, 2.0f, 1, 1, 1)},
            {"icosahedron", IcosahedronGeometry::generate_mesh()},
            {"dodecahedron", DodecahedronGeometry::generate_mesh()},
            {"octahedron", OctahedronGeometry::generate_mesh()},
            {"tetrahedron", TetrahedronGeometry::generate_mesh()},
            {"lathe", LatheGeometry::generate_mesh(LathePoints(), 3)},
            {"circle2", CircleGeometry::generate_mesh(1.0f, 16)},
            {"ring", RingGeometry::generate_mesh(0.5f, 1.0f, 32, 3, Degrees{0}, Degrees{180})},
            {"torus2", TorusGeometry::generate_mesh(0.9f, 0.1f, 12, 12, Degrees{360})},
            {"cylinder2", CylinderGeometry::generate_mesh(1.0f, 1.0f, 2.0f, 16, 1)},
            {"cone2", ConeGeometry::generate_mesh(1.0f, 2.0f, 16)},
            {"plane2", PlaneGeometry::generate_mesh(2.0f, 2.0f, 1, 1)},
            {"sphere2", SphereGeometry::generate_mesh(1.0f, 16, 16)},
        };
    }
}
