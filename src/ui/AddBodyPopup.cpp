...
#include "src/simtk_bindings/SimTKConverters.hpp"
...
std::optional<NewBody> osc::AddBodyPopup::draw(char const* modalName, OpenSim::Model const& model) {
    ...
    // prompt center of mass
    {
        ...
        ImGui::NextColumn();
        ui::lockable_f3_editor::draw("##comlockbtn", "##comeditor", st.com, &st.comLocked);
        ImGui::NextColumn();
    }
    ...
    // prompt inertia
    {
        ...
        ImGui::NextColumn();
        ui::lockable_f3_editor::draw("##inertialockbtn", "##intertiaeditor", st.inertia, &st.inertiaLocked);
        ImGui::NextColumn();
    }
