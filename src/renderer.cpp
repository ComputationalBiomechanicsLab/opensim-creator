...
struct Geometry_visitor final : public DecorativeGeometryImplementation {
	Model& model;
	State const& state;
	Geometry_loader_impl& impl;
	State_geometry& out;


	Geometry_visitor(Model& _model,
		State const& _state,
		Geometry_loader_impl& _impl,
		State_geometry& _out) :
		model{ _model },
		state{ _state },
		impl{ _impl },
...
Geometry_loader::Geometry_loader() : impl{ new Geometry_loader_impl{} } {
}
Geometry_loader::Geometry_loader(Geometry_loader&&) = default;
Geometry_loader& Geometry_loader::operator=(Geometry_loader&&) = default;
...
Geometry_loader::~Geometry_loader() noexcept = default;
...
void osmv::Renderer::draw(Application const& ui, OpenSim::Model& model, SimTK::State& s) {
	...
	for (Mesh_instance const& mi : state->geom.mesh_instances) {
		...
		state->geom_loader.load_mesh(mi.mesh_id, state->scratch.mesh);
		state->meshes.resize(std::max(state->meshes.size(), mi.mesh_id + 1));
		state->meshes[mi.mesh_id] = Mesh_on_gpu{ state->scratch.mesh };
	}
	...
	// depth test
	std::sort(state->geom.mesh_instances.begin(), state->geom.mesh_instances.end(), [](Mesh_instance const& a, Mesh_instance const& b) {
